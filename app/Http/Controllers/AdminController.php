...
class AdminController extends Controller
{
   ...
   public function editOfflineExam($id){
    ...
    {
        $payment_gateways = PaymentMethods::where('school_id', auth()->user()->school_id)
                                        ->where('name', 'paypal')
                                        ->get();
        ...
        if ($payment_gateways->count() > 0) {
            $paypal = $payment_gateways->first()->toArray();
            $paypal_keys = json_decode($paypal['payment_keys']);
        }
    
        return view('admin.payment_settings.key_settings', [
            'paypal' => $paypal,
    ...
    public function paymentSettings_post(Request $request)
    {
        ...
        if ($request->off_pay_ins_file || $request->off_pay_ins_text) {
            if ($request->off_pay_ins_file) {
                ...
                $newFileName = random(8).'.'.$ext;
                $request->off_pay_ins_file->move(public_path().'/assets/uploads/offline_payment/',$newFileName);
            ...
            return redirect()->back()->with('message','Offline payment instruction updated.');
        }
        ...
        return redirect()->route('admin.settings.payment')->with('message', 'Payment settings updated');
    }

    public function insert_gateways()
{   
    $paypal = PaymentMethods::where([
        'name' => 'paypal', 
        'school_id' => auth()->user()->school_id
    ])->first();
